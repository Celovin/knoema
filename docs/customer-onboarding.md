...
```bash
luvoire customer create \
  --tenant-id tenant_pro_demo \
  --tier pro \
  --display-name "Pro Demo"
```

The command prints a boxed secret once. Store the `Bearer luvoire_...` value immediately. The registry stores only the key hash.
...
```bash
export LUVOIRE_BEARER="luvoire_<api-key-id>_<secret>"
```
...
curl -s \
  ...
  -H "Authorization: Bearer ${LUVOIRE_BEARER}" \
...
The response includes a `simulation_id`. Authenticated responses include `X-Luvoire-Tenant-Tier: pro`.

7. Review usage, then rotate or revoke keys as needed:

```bash
luvoire customer show-usage --tenant-id tenant_pro_demo --month 2026-04
luvoire customer rotate-key --tenant-id tenant_pro_demo --api-key-id ak...
luvoire customer revoke-key --tenant-id tenant_pro_demo --api-key-id ak... --reason operator-request
```
...
| Command | Purpose |
| --- | --- |
| `luvoire customer create` | Create a tenant and first API key. |
| `luvoire customer issue-key` | Issue an additional key for an existing tenant. |
| `luvoire customer rotate-key` | Issue a replacement key and leave the old key on a 24-hour grace window. |
| `luvoire customer revoke-key` | Revoke a key immediately with an operator reason. |
| `luvoire customer show-usage` | Aggregate JSONL usage rows by model for a tenant and optional month. |
| `luvoire customer list` | List tenants with tier and key count. Secrets are excluded. |
