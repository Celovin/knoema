...
| Field | Value |
| --- | --- |
| Title | Luvoire: An Open Runtime for Persistent NPCs, Synthetic Replay Research, and Reproducible Agent Simulation |
...
| Sally-Anne benchmark | `src/luvoire/theory_of_mind.py` and `tests/test_phase43_theory_of_mind.py` | Yes | 20 deterministic false-belief and witnessed-move cases. |
...
| --- | --- | --- | --- | ---: | --- | --- |
| Luvoire deterministic local runtime | Social simulation | 500-agent metropolis logs | mean throughput actions/s | 1,657.527 | Higher | `experiments/500_agent_metropolis/results/summary.json` |
| Luvoire deterministic local runtime | Social simulation | 500-agent metropolis logs | mean p95 latency ms | 65.055 | Lower | `experiments/500_agent_metropolis/results/summary.json` |
| Luvoire persona opt-in ToM | Theory of mind | Sally-Anne benchmark | success rate | 1.000 | Higher | `tests/test_phase43_theory_of_mind.py` |
| Luvoire deterministic local runtime | Agent-based modeling | Schelling reproduction | threshold 0.3 segregation index | 0.548 | Target-band | `experiments/schelling_segregation/results/summary.json` |
| Luvoire deterministic local runtime | Agent-based modeling | Schelling reproduction | threshold 0.7 segregation index | 0.942 | Target-band | `experiments/schelling_segregation/results/summary.json` |
| Luvoire deterministic local runtime | Multi-agent cooperation | Axelrod tournament | Tit-for-Tat rank | 1 | Lower | `experiments/axelrod_prisoners_dilemma/results/summary.json` |
| Luvoire scoring utilities | Social simulation | 50-agent village log | PCS average | 0.948 | Higher | `benchmarks/scoring/results/metrics.json` |
| Luvoire scoring utilities | Social simulation | 500-agent metropolis representative log | RCS average | 0.760 | Higher | `benchmarks/scoring/results/metrics.json` |

## Comparison Notes

Stanford Generative Agents and Concordia are relevant comparison systems for generative social simulation, but this repository does not report their throughput, Sally-Anne, PCS, or RCS numbers because equivalent external runs have not been performed locally. Mesa, NetLogo, GAMA, Repast, Swarm, MASON, and AnyLogic are ABM references; Luvoire compares against their design lineage rather than claiming measured superiority.
...
All public-safety examples are fictional, synthetic, and non-identifying. Luvoire does not support prediction, suspect scoring, surveillance, enforcement automation, or profiling of identifiable people.
