...
Luvoire observability is opt-in. By default, the API server does not expose `/metrics`, does not configure tracing, and does not export request or tenant data.
...
```bash
LUVOIRE_METRICS_ENABLED=1 uvicorn luvoire.api.server:app --host 127.0.0.1 --port 8000
```
...
| Metric | Labels | Notes |
| --- | --- | --- |
| `luvoire_requests_total` | `route`, `method`, `status`, `tier` | API request counts. |
| `luvoire_request_duration_seconds_bucket` | `route` | Route-level latency histogram buckets. |
| `luvoire_billing_tokens_total` | `tenant_id_hash`, `direction`, `tier` | Input and output token counts. Tenant IDs are hashed before labeling. |
| `luvoire_webhook_dispatch_total` | `status` | Webhook delivery, failure, and exhaustion counts. |
| `luvoire_rate_limit_hits_total` | `tier` | Tier rate-limit denials. |
