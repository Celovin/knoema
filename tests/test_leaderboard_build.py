...
def test_leaderboard_knoema_submission_is_generated_from_summaries() -> None:
    generated = build_leaderboard.generate_knoema_submission()
    committed = yaml.safe_load(
        Path("bench/submissions/knoema-0.2.0.yaml").read_text(encoding="utf-8")
    )

    assert committed == generated
    assert generated["framework"] == "Knoema Engine"
...
def test_leaderboard_build_is_deterministic_and_puts_knoema_first() -> None:
    ...
    assert table_rows[2].startswith("| 1 | Knoema Engine |")
    for axis in build_leaderboard.AXES:
        assert f"{axis.label} caveat" in table_rows[0]


def test_validate_submission_cli_accepts_knoema_entry_and_rejects_missing_axis(
    tmp_path: Path,
) -> None:
    valid = subprocess.run(
        [sys.executable, "scripts/validate_submission.py", "bench/submissions/knoema-0.2.0.yaml"],
        check=False,
        capture_output=True,
        text=True,
    )
    ...
    broken = build_leaderboard.generate_knoema_submission()
