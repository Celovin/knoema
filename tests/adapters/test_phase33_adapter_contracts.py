...
def test_phase33_unity_and_godot_clients_share_core_payload_fields() -> None:
    unity_client = Path("adapters/unity/Runtime/KnoemaClient.cs").read_text(encoding="utf-8")
    godot_client = Path("adapters/godot/scripts/knoema_client.gd").read_text(encoding="utf-8")
...
def test_phase33_adapter_docs_keep_package_and_runtime_entrypoints() -> None:
    ...
    assert "https://github.com/Celovin/knoema.git?path=adapters/unity" in unity_readme
    assert "addons/knoema" in godot_readme
