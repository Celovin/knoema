# Replay Demo Changelog

## 2026-04-22

- Completed the CAT-28 tier 5 personality taxonomy with 22 single archetypes and 6 composite gestalt overlays.
- Split CAT-28 replay viewer options into single and composite groups while preserving the existing pedagogical notice flow.
- Added the 10000-agent 60x60 local replay scenario for the largest current
  offline city-scale demo path.
- Added the 5000-agent 40x40 local replay scenario for demo-scale playback between
  the 1000-agent scenario and future larger-scale runs.
- Added CAT-28 tier 5 personality overlays to the offline replay viewer's archetype injection dropdown.
- Grouped the dropdown into criminology tier 1, criminology tier 2 historical, and CAT-28 personality sections with a visual separator before the personality overlays.
- Extended the pedagogical notice with a personality-specific paragraph for first-time tier 5 injection in each browser session.
